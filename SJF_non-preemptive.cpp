// C++ program to implement Shortest Job first Non-Preemptive with different Arrival Time
...
public:
	...
	void display(int n)
	{
		...
		{ // display this when procceses are arranged.
			cout<<"\n-----------------------Final Result--------------------\n\n";
			cout<<"Process ID\tArrival Time\tBurst Time\tWaiting Time\tTurnaround Time\n";
			for(int i=0; i<num; i++)
			{
				cout<<pros[i][0]<<"\t\t"<<pros[i][1]<<"\t\t"<<pros[i][2]<<"\t\t"<<pros[i][4]<<"\t\t"<<pros[i][5]<<"\n";
				avgWT+=pros[i][4];	// add all WT
				avgTAT+=pros[i][5]; // add all TAT
			}
			cout<<endl;
			cout<<"avg. Waiting Time = "<<(avgWT/num);
			cout<<endl;
			cout<<"avg. Turn Around Time = "<<(avgTAT/num);
		}
	}
